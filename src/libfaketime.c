...
static void ft_shm_create(void) {
  ...
  if (SEM_FAILED == (semN = sem_open(sem_name, O_CREAT|O_EXCL, S_IWUSR|S_IRUSR|S_IRGRP|S_IWGRP, 1)))
  { /* silently fail on platforms that do not support sem_open() */
    return;
  }
  /* create shm */
  if (-1 == (shm_fdN = shm_open(shm_name, O_CREAT|O_EXCL|O_RDWR, S_IWUSR|S_IRUSR|S_IRGRP|S_IWGRP)))
  {
...
static void ft_shm_init (void)
{
  ...
  if (ft_shared_env != NULL)
  {
    ...
    if (-1 == (ticks_shm_fd = shm_open(shm_name, O_CREAT|O_RDWR, S_IWUSR|S_IRUSR|S_IRGRP|S_IWGRP)))
    {
